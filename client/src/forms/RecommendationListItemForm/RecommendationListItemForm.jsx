...
function RecommendationListItemForm({ recommendation }) {
  ...
  const { deleteRecommendationsById, upvoteRecommendation } =
    useRecommendation();

  return (
    <Card
      className="recommendation-card card-custom mx-auto"
      style={{ maxWidth: "40vw" }}
    >
      <Card.Img
        variant="top"
        src={
          recommendation.foto ? `${baseURL}/${recommendation.foto}` : "view.jpg"
        }
